...
int internal_fg(char **args) {
    ...
    if((pos >= n_pids+1)||(pos == 0)) {
        ...
        fprintf(stderr, "internal_fg() --> NO EXISTE ESTE TRABAJO\n");
        return pos;
    }
    //SI SU STATUS ES 'D'
    if (jobs_list[pos].status == 'D') {
        ...
        sprintf(mensaje, "internal_fg() --> SEÑAL %d ENVIADA A %d DE NOMBRE (%d)\n" RESET_FORMATO, SIGCONT, jobs_list[pos].pid, jobs_list[pos].status);
        write(2, mensaje, strlen(mensaje));
    }
    ...
    //MOSTRAR POR PANTALLA EL cmd
    sprintf(mensaje, "EL cmd SIN '&' ES: [%s]\n"RESET_FORMATO, jobs_list[pos].cmd);
    write(2, mensaje, strlen(mensaje));
...
int internal_bg(char **args) {
    ...
    if((pos >= n_pids+1)||(pos == 0)) {
        ...
        sprintf(mensaje, VERDE_T "internal_bg() --> [%d] %d   %c  %s\n\n" RESET_FORMATO, pos, jobs_list[pos].pid, jobs_list[pos].status, jobs_list[pos].cmd);
        write(2, mensaje, strlen(mensaje));
    }
    return 1;
}
    ...
    if (parse_args(args, line) > 0) {
        ...
        if (check_internal(args)) {
            ...
            if (pid == 0) {
                ...
                sprintf(mensaje,ROJO_T "---ERROR AL HACER execvp() EN execute_line(char *line)---");
                write(2,mensaje,strlen(mensaje));
                //realizar un exit(-1);
                exit(-1);
                ...
                if (tipo == 1) {
                    ...
                    fprintf(stderr, "[execute_line()→ PID padre: %d nombre: (%s)]\n" RESET_FORMATO, getpid(),command_line);
                    fprintf(stderr, "[execute_line()→ PID hijo: %d nombre: (%s)]\n" RESET_FORMATO, jobs_list[n_pids].pid,jobs_list[0].cmd);
                    internal_jobs(args);
                } else if (tipo == 0) {
                    //---------ES FOREGROUND---------
                    fprintf(stderr, "[execute_line()→ PID padre: %d nombre: (%s)]\n" RESET_FORMATO, getpid(),command_line);
...
void reaper(int signum) {
    ...
    while ((acabado=waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        if(jobs_list[0].pid == acabado) {
            ...
            //COMENTARIOS DE DEBUGGING
            if (WIFEXITED(status)) {
                sprintf(mensaje, VERDE_T "reaper() --> Proceso hijo %d (%s) finalizado con exit(), en estado: %d]\n" RESET_FORMATO, jobs_list[0].pid, jobs_list[0].cmd, WEXITSTATUS(status));
                write(2,mensaje,strlen(mensaje));
            } else if (WIFSIGNALED(status)) {
                sprintf(mensaje, VERDE_T "reaper() --> Proceso hijo %d (%s) finalizado al recibir señal %d]\n" RESET_FORMATO, jobs_list[0].pid, jobs_list[0].cmd, WTERMSIG(status));
                write(2,mensaje,strlen(mensaje));
            }
            ...
            //---------------BACKGROUND---------------
            //COMENTARIOS DE DEBUGGING
            //IMPRIMIR POR SALIDA ESTANDAR DE ERRORES QUE EL PROCESO HA ACABADO Y LOS DATOS DE ESTE PROCESO
            sprintf(mensaje, VERDE_T "reaper() --> HA FINALIZADO EL PROCESO CON pid %d DE NOMBRE %s EN ESTADO %d\n" RESET_FORMATO, jobs_list[n_pids].pid, jobs_list[0].cmd, WTERMSIG(status));
            write(2,mensaje,strlen(mensaje));
            if (WIFEXITED(status)) {
                sprintf(mensaje, VERDE_T "reaper() --> Proceso hijo %d (%s) finalizado con exit(), en estado: %d]\n" RESET_FORMATO, jobs_list[0].pid, jobs_list[0].cmd, WEXITSTATUS(status));
                write(2,mensaje,strlen(mensaje));
...
void ctrlz(int signum) {
    ...
    if (jobs_list[0].pid > 0) {
        ...
        if (comparacion != 0) {
            ...
            //MOSTRAR ERROR
            sprintf(mensaje, GRIS "ctrlz()→ SEÑAL %d NO ENVIADA AL PROCESO %d (%s) YA QUE ESTE ES EL SHELL\n" RESET_FORMATO, SIGSTOP, getpid(), mi_shell);
            write(2,mensaje,strlen(mensaje));
        }
    } else {
        //SI NO ES EN FOREGROUND
        sprintf(mensaje, GRIS "ctrlz()→ SEÑAL %d NO ENVIADA AL PROCESO %d (%s) YA QUE ESTE NO ES EN FOREGROUND\n" RESET_FORMATO, SIGSTOP, getpid(), mi_shell);
        write(2,mensaje,strlen(mensaje));
    }
}
...
int is_output_redirection (char **args) {
    ...
    if (encontrado == 1) {
        ...
        //COMPROBAR SI HAY ERROR AL ABRIR FICHERO
        if (fd == -1) {
            //HAY ERROR
            fprintf(stderr, "is_output_redirection() --> ERROR AL ABRIR EL FICHERO");
        }
        dup2(fd,1);
        int error = close(fd);
        //GESTIONAR ERRORE AL CERRAR FICHERO 
        if(error == -1) {
            //HAY ERROR
            fprintf(stderr, "is_output_redirection() --> ERROR AL CERRAR EL FICHERO");
        }
    }
    fprintf(stderr, "is_output_redirection() --> FINAL DE LA FUNCIÓN");
